...
import { detectOverdueAp, detectOverdueServices } from '@/features/workspace/workspace-service';
...
function contactName(contacts: Contact[], contactId: string | null | undefined, fallback = 'Unknown'): string {
...
export function detectApReturnDueSoon(apRecords: ApRecord[], contacts: Contact[]): NotificationCandidate[] {
...
export function detectApPaymentOverdue(apRecords: ApRecord[], contacts: Contact[]): NotificationCandidate[] {
...
export function detectPaymentsDueSoon(receivables: Receivable[], contacts: Contact[]): NotificationCandidate[] {
  const target = startOfDay(addDays(new Date(), DUE_SOON_DAYS));
  return receivables
    .filter((r) => {
...
export function detectPaymentsOverdue(receivables: Receivable[], contacts: Contact[]): NotificationCandidate[] {
  return receivables
    .filter((r) => {
      if (r.status === 'paid') return false;
      const remaining = r.amount - r.amountReceived;
...
export function buildApOverdueCandidates(apRecords: ApRecord[], contacts: Contact[]): NotificationCandidate[] {
...
export function buildChequeMaturingCandidates(cheques: Cheque[], contacts: Contact[]): NotificationCandidate[] {
...
export function buildGemTrackNotificationCandidates(input: {
  ...
  return [
    ...
    ...buildServiceOverdueCandidates(input.services, input.contacts, input.gems),
    ...detectPaymentsDueSoon(input.receivables, input.contacts),
    ...detectPaymentsOverdue(input.receivables, input.contacts),
  ];
}

export function notificationDedupeKey(n: Pick<AppNotification, 'type' | 'referenceType' | 'referenceId'>): string {
  return `${n.type}:${n.referenceType ?? ''}:${n.referenceId ?? ''}`;
}

export function existingGemTrackNotificationKeys(notifications: AppNotification[]): Set<string> {
  return new Set(
    notifications.filter((n) => isGemTrackNotificationType(n.type)).map(notificationDedupeKey),
  );
}
