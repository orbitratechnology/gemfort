...
export default function TripDetailScreen() {
  ...
  async function handleStatus(next: 'ongoing' | 'completed' | 'cancelled') {
    ...
    try {
      ...
      toast.success(`Trip marked as ${TRIP_STATUS_LABELS[next].toLowerCase()}.`);
    } catch (e) {
  ...
  async function handleRecordSale() {
    ...
    setLoading(true);
    try {
      await recordTripGemSale(user.uid, trip.id, saleTarget.id, saleTarget.gemId, price);
  ...
  const budgetPct = budgetUsedPercent(trip, summary.totalExpenses + summary.purchaseSpend);
      ...
      <ThemedScrollView contentContainerStyle={styles.content}>
        <View style={[styles.hero, { backgroundColor: colors.primary }]}>
          <View style={styles.heroTop}>
            <View style={[styles.heroIcon, { backgroundColor: colors.onPrimary + '22' }]}>
              <Icon name={typeMeta?.icon ?? 'flight'} size={26} color={colors.onPrimary} />
            </View>
          ...
          <Text style={[styles.heroDates, { color: colors.onPrimary + 'AA' }]}>{formatTripDates(trip)}</Text>
        ...
        <View style={[styles.card, { backgroundColor: colors.surfaceContainerLowest }]}>
          <Text style={[styles.cardTitle, { color: colors.primary }]}>Summary</Text>
          <View style={styles.statGrid}>
            <Stat label="Expenses" value={formatCurrency(summary.totalExpenses)} colors={colors} />
            <Stat label="Purchases" value={String(summary.totalGemsPurchased)} colors={colors} />
            <Stat label="Sold" value={String(summary.totalGemsSold)} colors={colors} />
            <Stat
              label="Net result"
          ...
          {trip.budget > 0 ? (
            <View style={styles.budgetWrap}>
              <View style={styles.budgetRow}>
                ...
                <Text style={[styles.budgetPct, { color: colors.onSurface }]}>{budgetPct}%</Text>
              </View>
              <View style={[styles.budgetTrack, { backgroundColor: colors.surfaceContainerHigh }]}>
                <View
                  style={[
                    styles.budgetFill,
                    {
                      width: `${budgetPct}%`,
                      backgroundColor: budgetPct > 90 ? colors.error : colors.primary,
                    },
                  ]}
                />
              </View>
        ...
        <View style={styles.linkRow}>
          <Pressable
            ...
            style={({ pressed }) => [styles.linkBtn, { backgroundColor: colors.surfaceContainerLow }, pressed && { opacity: 0.85 }]}>
            <Icon name="receipt" size={20} color={colors.primary} />
            <Text style={[styles.linkLabel, { color: colors.onSurface }]}>Add expense</Text>
          </Pressable>
          {isSourcing ? (
            <Pressable
              onPress={() => router.push(`${base}/add-purchase` as never)}
              style={({ pressed }) => [styles.linkBtn, { backgroundColor: colors.secondaryContainer }, pressed && { opacity: 0.85 }]}>
              <Icon name="diamond" size={20} color={colors.onSecondaryContainer} />
              <Text style={[styles.linkLabel, { color: colors.onSecondaryContainer }]}>Buy gem</Text>
            </Pressable>
          ) : null}
          {isSelling ? (
            <Pressable
              onPress={() => router.push(`${base}/add-gems` as never)}
              style={({ pressed }) => [styles.linkBtn, { backgroundColor: colors.primaryContainer }, pressed && { opacity: 0.85 }]}>
              <Icon name="inventory-2" size={20} color={colors.onPrimaryContainer} />
              <Text style={[styles.linkLabel, { color: colors.onPrimaryContainer }]}>Add gems</Text>
            </Pressable>
          ) : null}
        </View>

        {isSourcing && summary.totalGemsPurchased > 0 && summary.totalExpenses > 0 ? (
          <Pressable
            ...
            <Text style={[styles.overheadText, { color: colors.primary }]}>Distribute overhead to gems</Text>
          </Pressable>
        ) : null}

        {saleTarget ? (
          <View style={[styles.saleCard, { backgroundColor: colors.surfaceContainerLowest, borderColor: colors.primary }]}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Expenses</Text>
          {expenses.length === 0 ? (
            <Text style={[styles.empty, { color: colors.textMuted }]}>No expenses logged yet.</Text>
          ) : (
            expenses.map((e) => (
              <View
                key={e.id}
                style={[styles.listRow, { backgroundColor: colors.surfaceContainerLowest, borderColor: colors.outlineVariant }]}>
                <View style={[styles.listIcon, { backgroundColor: colors.primary + '14' }]}>
                  <Icon name={getExpenseCategoryIcon(e.category)} size={18} color={colors.primary} />
                </View>
                <View style={styles.listBody}>
                  <Text style={[styles.listTitle, { color: colors.onSurface }]}>
                    {getExpenseCategoryLabel(e.category)}
                  </Text>
                  <Text style={[styles.listSub, { color: colors.textMuted }]} numberOfLines={1}>
                    {e.description || formatDate(e.date)}
                  </Text>
                </View>
                <Text style={[styles.listAmt, { color: colors.onSurface }]} selectable>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Gems</Text>
          {tripGems.length === 0 ? (
            <Text style={[styles.empty, { color: colors.textMuted }]}>No gems linked to this trip yet.</Text>
          ) : (
            tripGems.map((tg) => {
              const gem = gemMap.get(tg.gemId);
              const showSale =
                canRecordSales && tg.role === 'parcel' && tg.status === 'on_trip' && saleTarget?.id !== tg.id;

              return (
                <View key={tg.id} style={styles.gemRowWrap}>
                  <Pressable
                    onPress={() => router.push(`/(marketplace)/(tabs)/workspace/gems/${tg.gemId}` as never)}
                    style={({ pressed }) => [
                      styles.listRow,
                      { backgroundColor: colors.surfaceContainerLowest, borderColor: colors.outlineVariant },
                      pressed && { opacity: 0.85 },
                    ...
                    <View style={[styles.listIcon, { backgroundColor: colors.secondaryContainer }]}>
                      <Icon name="diamond" size={18} color={colors.onSecondaryContainer} />
                    </View>
                    <View style={styles.listBody}>
                      <Text style={[styles.listTitle, { color: colors.onSurface }]}>
                        {gem?.sku ?? tg.gemId.slice(0, 8)} · {tg.role === 'purchase' ? 'Purchase' : 'Parcel'}
                      </Text>
                      <Text style={[styles.listSub, { color: colors.textMuted }]}>
                        {gem ? `${formatGemType(gem.gemType)} · ${gem.currentWeight}ct` : tg.status}
                      </Text>
                    </View>
                    <Text style={[styles.listAmt, { color: colors.primary }]} selectable>
                      {tg.salePrice != null
                  ...
                  {showSale ? (
                    <Pressable
                      ...
                      <Icon name="sell" size={16} color={colors.onPrimaryContainer} />
                      <Text style={[styles.saleBtnText, { color: colors.onPrimaryContainer }]}>Record sale</Text>
                    </Pressable>
                  ) : null}
                </View>
              );
        ...
        {trip.notes ? (
          <View style={[styles.card, { backgroundColor: colors.surfaceContainerLowest }]}>
            ...
            <Text style={[styles.notes, { color: colors.onSurfaceVariant }]} selectable>
              {trip.notes}
            </Text>
          </View>
        ) : null}
...
function Stat({
  ...
  return (
    <View style={styles.stat}>
      ...
      <Text style={[styles.statValue, { color: accent ?? colors.onSurface }]} selectable>
        {value}
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  stat: { width: '47%', gap: 2 },
  statLabel: { ...Typography.labelMd, textTransform: 'uppercase', letterSpacing: 0.8 },
  statValue: { ...Typography.headlineMdMobile, fontWeight: '700', fontVariant: ['tabular-nums'] },
  ...
  listAmt: { ...Typography.labelMd, fontWeight: '700', fontVariant: ['tabular-nums'] },
  saleBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
