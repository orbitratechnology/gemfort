...
import { formatTripDates, getTripsByStatus } from '@/features/workspace/trip-utils';
...
function TripRow({ trip, colors, onPress }: { trip: Trip; colors: ReturnType<typeof useAppTheme>['colors']; onPress: () => void }) {
  const typeMeta = TRIP_TYPES.find((t) => t.id === trip.tripType) ?? TRIP_TYPES[0];
  ...
  return (
    <Pressable
      ...
      <View style={[styles.rowIcon, { backgroundColor: isActive ? colors.primaryContainer : colors.surfaceContainerHigh }]}>
        <Icon name={typeMeta.icon} size={22} color={isActive ? colors.onPrimaryContainer : colors.onSurfaceVariant} />
      </View>
      <View style={styles.rowBody}>
        <View style={styles.rowTop}>
          ...
          <View style={[styles.badge, { backgroundColor: colors.surfaceContainerHighest }]}>
        ...
        {trip.summary.netResult !== 0 || trip.summary.totalExpenses > 0 ? (
          <Text
            style={[
              styles.rowNet,
              { color: trip.summary.netResult >= 0 ? colors.successEmerald : colors.error },
            ]}
            selectable>
...
export default function TripsScreen() {
  ...
  const { data: trips = [], refetch, isRefetching } = useQuery({
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']}>
      <StackHeader
        ...
        right={
          <Pressable
            onPress={() => router.push('/(marketplace)/(tabs)/workspace/trips/add' as never)}
      ...
      <ScrollView
        ...
        <Text style={[styles.subtitle, { color: colors.textMuted }]}>
          Track sourcing and selling trips — expenses, purchases, and sales in one place.
        </Text>
        ...
        <Pressable
          ...
          <View style={styles.createText}>
            <Text style={[styles.createTitle, { color: colors.onPrimary }]}>Plan a new trip</Text>
            <Text style={[styles.createSub, { color: colors.onPrimary + 'AA' }]}>
          ...
          <>
            {active.length > 0 ? (
              <View style={styles.section}>
                <Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Active</Text>
                {active.map((t) => (
                  <TripRow
                    key={t.id}
                    trip={t}
                    colors={colors}
                    onPress={() => router.push(`/(marketplace)/(tabs)/workspace/trips/${t.id}` as never)}
                  />
                ))}
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  rowNet: { ...Typography.labelMd, fontWeight: '700', marginTop: 2, fontVariant: ['tabular-nums'] },
});
